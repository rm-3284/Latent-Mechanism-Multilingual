...
from lib.circuit_tracer_import import Feature, ReplacementModel
from lib.models import hf_model_names


def coerce_intervention_value(val) -> float:
    """Convert intervention magnitudes to plain floats (rejects Tensor leakage)."""
...
def normalize_intervention_by_lang(
    ...
    return {
        lang: normalize_intervention_list(intervention_by_lang[lang])
        for lang in intervention_by_lang
    }


def load_nnsight_model(model_name: str, device_map: str = "cpu"):
    """Load nnsight LanguageModel. Prefer CPU to avoid a second full model on GPU."""
    import nnsight
...
def ablation(supernode_dict: dict[str, list[tuple[Feature, float]]], lang: str, alpha: float=0) -> list[tuple[int, int, int, float]]:
    ...
    for feature, val in supernode_dict[lang]:
        ...
        ablation_value = coerce_intervention_value(val * alpha)
...
def amplification(supernode_dict: dict[str, list[tuple[Feature, float]]], lang: str) -> list[tuple[int, int, int, float]]:
    ...
    for feature, val in supernode_dict[lang]:
        ...
        amplification_value = coerce_intervention_value(val)
